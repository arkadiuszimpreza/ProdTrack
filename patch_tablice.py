...
with open('src/components/production/OperatorPanelTablice.tsx', 'r') as f:
    content = f.read()
...
# Make the drawing container grow but also have a min-height on mobile
content = content.replace(
...
# Close div for table wrapper
# It is just before </div> </div> </div> )}
# Let's find the table end
content = content.replace(
    '</table>',
    '</table></div>'
)
...
with open('src/components/production/OperatorPanelTablice.tsx', 'w') as f:
    f.write(content)
