...
const fixRegex = /\{guessPrefix\(b\.articleName \|\| ''\) !== 'BL' && \([\s\S]*? handleCalcChange\(b\.id as string, 'length', e\.target\.value, b\)\}\s*className=\{cn\([^]+\)\}\s*\/>/;

const correctUI = `{guessPrefix(b.articleName || '') !== 'BL' && (
                                   <>
...
c = c.replace(fixRegex, correctUI);
